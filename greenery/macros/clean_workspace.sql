{# https://github.com/randypitcherii/dbt_workspace/blob/production/dbt/macros/snowflake_utils/cleanup/clean_workspace.sql #} 

{# clean_workspace

This macro drops all the schemas within a database to "clean" the workspace. 
Use the dry_run param to see the schemas that will be dropped before dropping them.
The schemas to drop will be those that match the schema_like (if provided) 
AND do not match the schema_not_like (if provided). 
The information_schema is always
